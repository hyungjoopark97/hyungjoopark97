...
public class Application3 {
    ...
    public static void main(String[] args){
        ...
        * 1-2 예약어는 사용이 불가능하다(자바에서 미리 사용하겠다고 컴파일러와 예약한 키워드를 예약이라고 한다.
        * 1-3 변수명은 대/소문자를 구분한다 (자바는 대/소문자를 구분하는 언어이다)
        * 1-4 변수명은 숫자로 시작할 수 없다 (시작만 할 수 없고 숫자를 섞어서 사용하는 것은 가능하다)
        * 1-5 특수기호는 '_'와 '$'만 사용이 가능하다(따라서 공백은 사용할 수 없다)
        *
        *
        * 2. 컴파일 에러를 발생시키지는 않지만 개발자들끼리의 암묵적인 규칙
        * 2-1 변수명의 길이에 제한은 없다(하지만 적당히 하자)
        * 2-2 변수명이 합성어로 이루어진 경우 첫 단어는 소문자 두 번째로 시작하는 단어는 대문자로 시작한다(camel-case)
        * 2-3 단어와 단어 사이의 연결은 언더스코어(_)로 사용하지 않는다(타 언어의 네이밍 규칙 때문이다)
        ...
        * 2-8 boolean 형은 의문문으로 가급적 긍정형태로 네이밍 한다. */
        ...
        /* 목차 1-5 특수기호는 '_'과 '$'만 사용 가능하다. */
        //int a@ge = 20; // 사용 가능한 늑수문자 외에는 사용이 불가능
        int _age = 20; // 언더스코어(_)는 사용가능함, 처음도 가능하고 중간이나 끝에도 가능함.
        int a$ge = 20; // $사용 가능함. 처음도 가능하고 중간이나 끝에도 가능함.

        /* 2. 컴파일 에러를 발생시키지는 않지만 개발자들끼리의 암묵적인 규칙*/
        /* 목차 2-1 변수명의 길이제한은 없다. 하지만 적당히 하자.*/
        int aalksjfhaslkdfjhaskdfjhaslkdfjhasdflkjh;

        /* 목차 2-2 변수명이 합성어로 이루어진 경우 첫 단어는 소문자 두 번째 시작 단어는 대문자로 시작한다 (카멜 표기법)*/
        /* 자바에서는 클래스명만 유일하게 대문자로 시작한다. */
        int maxAge = 20;
        int minAge = 20;

        /* 목차 2-3 단어와 단어 사이는 언더스코어(_)로 하지 않는다 */
        String user_name; // 에러가 발생하지는 않지만 이렇게 하면 안된다.
        String userName; // 이것이 올바른 표기법이다.
        ...
        boolean notAlive = false;  // 부정형보다 긍정형이 더 나은 방식이다.
