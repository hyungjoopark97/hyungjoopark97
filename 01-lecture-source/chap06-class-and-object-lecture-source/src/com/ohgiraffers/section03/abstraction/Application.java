...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        * 유연성 확보는 여러곳에 적용할 수 잇는 유일한 객체를 의미하며, 즉, 재사용성이 높아질 수 있다는 의미이다.
        ...
        *  - 사용자, 카레이서, 자동차,
        *  2. 객체간의 상호작용
        *   -카레이서가 수신할 수 있는 메시지(카레이서가 해야 하는 일)
        * 1. 시동을 걸어라.
        * 2. 엑셀을 밟아라
        * 3. 브레이크를 밟아라
        * 4. 시동을 꺼라.
		...
		* */
		...
		CarRacer carRacer = new CarRacer(); //카레이서를 조종하기 위해 클래스의 객체를 생성해준다.
		Scanner sc = new Scanner(System.in); //이를 콘솔에서 컨트롤하기 위해 스캐너를 달아준다.
		...
		while (true){
			...
			int no = sc.nextInt(); //넘버를 넣어 기능을 구현한다. 스캐너를 통해 직접 기능을 작동하도록 한다.

			switch(no){ //break문을 잊지 않도록 한다.
			...
			if(no==9){
				...
				break; //프로그램이 꺼졌기 때문에 소스에서 빠져나가서 더 이상 소스가 실행하지 않도록 한다.
